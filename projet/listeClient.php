...
<div class="container">
    ...
    <h2 class="text-center text-capitalize text-uppercase">Liste des Clients</h2>
    <hr>
    ...
    <?php
        ...
        {
    ?>
        <table  border="2px" class=" table table-striped table-hover  text-center text-capitalize  ">
            ...
            $pdo = $cnx->CNXbase();
            $type = "client";
            $requete = $pdo->prepare("SELECT nom ,prenom  , username , tel , type  FROM  utilisateur WHERE  type = :type");
            $requete->bindParam(':type', $type);
            $requete->execute();
            foreach ($requete as $row) { ?>

                <tr class="border border-danger" >
                    <td class="border border-danger" ><?php echo $row['nom'] ?></td>
                    <td class="border border-danger"><?php echo $row['prenom'] ?></td>
                    <td class="border border-danger"><?php echo $row['username'] ?></td>
                    <td class="border border-danger"><?php echo $row['tel'] ?></td>
                    <td class="border border-danger"><a style="background-color: #ffcccc;" class="btn " type="button" data-bs-toggle="collapse" data-bs-target="#collapseWidthExample" aria-expanded="false" aria-controls="collapseWidthExample" href="deleteclient.php?username=<?= $row['username']; ?>">Supprimer</a>
